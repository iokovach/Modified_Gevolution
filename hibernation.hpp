...
void writeRestartSettings(metadata & sim, icsettings & ic, cosmology & cosmo, const double a, double &dm, double &rad, double &dcdm, const double tau, const double dtau, const int cycle, const int restartcount = -1)
{
	...
	else
	{
		...
		fprintf(outfile, "tau_in                = %.15le\n", tau);
		fprintf(outfile, "restarted               = y\n");
		...
		fprintf(outfile, "dcdm_fraction   = %.15le\n", cosmo.dcdm_fraction);
		fprintf(outfile, "rate   = %.15le\n", cosmo.rate*C_SPEED_OF_LIGHT/sim.boxsize);
		...
		if (sim.snap_flag >0)
			{fprintf(outfile, "snapshot for every step = y\n");}
		else
			{fprintf(outfile, "snapshot for every step = n\n");} 
		
		if (sim.pk_flag >0)
			{fprintf(outfile, "pk for every step = y\n");}
		else
			{fprintf(outfile, "pk for every step = n\n");} 
		...
		COUT<<"restart cycle" << sim.restart_cycle<<endl;
		fprintf(outfile, "restart cycle      = %lg\n", sim.restart_cycle);
		fprintf(outfile, "restart path       =%s\n", sim.restart_path);
...
void hibernate(metadata & sim, icsettings & ic, cosmology & cosmo, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, Field<Real> & phi, Field<Real> & chi, Field<Real> & Bi, const double a, double &dm, double &rad, double &dcdm, const double tau, const double dtau, const int cycle, const int restartcount = -1)
{
	string h5filename;
	char buffer[5];
	int i;
	...
	writeRestartSettings(sim, ic, cosmo, a, dm, rad, dcdm,tau, dtau, cycle, restartcount);
