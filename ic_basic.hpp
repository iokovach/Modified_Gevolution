...
void generateIC_basic(metadata & sim, icsettings & ic, cosmology & cosmo, const double fourpiG, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, double * maxvel, Field<Real> * phi, Field<Real> * chi, Field<Real> * Bi, Field<Real> * source, Field<Real> * Sij, Field<Cplx> * scalarFT, Field<Cplx> * BiFT, Field<Cplx> * SijFT, PlanFFT<Cplx> * plan_phi, PlanFFT<Cplx> * plan_chi, PlanFFT<Cplx> * plan_Bi, PlanFFT<Cplx> * plan_source, PlanFFT<Cplx> * plan_Sij, parameter * params, int & numparam)
{
	...
	if (ic.pkfile[0] != '\0')	// initial displacements & velocities are derived from a single power spectrum
	{
		...
		generateDisplacementField(*scalarFT, sim.gr_flag * Hconf(a, dm, rad, dcdm, fourpiG, cosmo) * Hconf(a, dm, rad, dcdm, fourpiG, cosmo), pkspline, (unsigned int) ic.seed, ic.flags & ICFLAG_KSPHERE);
	}
	else					// initial displacements and velocities are set by individual transfer functions
	{
		...
		if (sim.baryon_flag == 2)	// baryon treatment = blend; compute displacement & velocity from weighted average
		{
			...
			else
			{
				for (i = 0; i < tk_d1->size; i++) 
					...
					temp1[i] = nbspline->y[i] - (((dm+dcdm) * tk_d1->y[i] + cosmo.Omega_b * tk_d2->y[i]) / ((dm+dcdm) + cosmo.Omega_b)) * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i];
			}
			
			if (sim.gr_flag > 0 || vnbspline == NULL) 
			{
				for (i = 0; i < tk_d1->size; i++)
					temp2[i] =  -a * (((dm+dcdm) * tk_t1->y[i] + cosmo.Omega_b * tk_t2->y[i]) / ((dm+dcdm) + cosmo.Omega_b)) * M_PI * 
					sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i];
			}
			else //vnbspline is only not NULL if you use class. 
			{
				for (i = 0; i < tk_d1->size; i++)
		...
		outfile << "# k-value, pkspline(k))" << std::endl;
	
		// Define k values for which we want to evaluate the spline
	...
	COUT << " " << sim.numpcl[0] << " total mass in CDM particles = " << pcls_cdm_info.mass * sim.numpcl[0] << endl;
	free(pcldata);
	...
	if (ic.pkfile[0] == '\0')	// set velocities using transfer functions
	{
		...
		scalarFT->saveHDF5(std::string(sim.output_path) + "/scalarFT_init.h5");
		...
		chi->saveHDF5(std::string(sim.output_path) + "/chi_init.h5");
		maxvel[0] = pcls_cdm->updateVel(initialize_q_ic_basic, 1., &chi, 1) / a;	// set CDM velocities

		COUT << "tk maxvel is" << maxvel[0] << endl;
	...
	{	
		rescale = a / Hconf(a, dm, rad, dcdm, fourpiG, cosmo) / (1.5 * Omega_m(a, dm, rad, dcdm, fourpiG, cosmo) + 2. * Omega_rad(a, dm, rad, dcdm, fourpiG, cosmo));
	...
	for (p = 0; p < cosmo.num_ncdm; p++)
	{
		...
		{		
			rescale = a / Hconf(a, dm, rad, dcdm, fourpiG, cosmo) / (1.5 * Omega_m(a, dm, rad, dcdm, fourpiG, cosmo) + Omega_rad(a, dm, rad, dcdm, fourpiG, cosmo));
