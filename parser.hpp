...
int parseMetadata(parameter * & params, const int numparam, metadata & sim, cosmology & cosmo, icsettings & ic)
{
	...
	if (parseParameter(params, numparam, "restarted", par_string))
	{
		if (par_string[0]=='Y' || par_string[0]=='y')
		{
			sim.restart_flag = 1;
	...
	parseParameter(params, numparam, "tau_in", ic.tau_in);
	...
	if (ic.generator == ICGEN_READ_FROM_DISK)
	{
		...
		parseParameter(params, numparam, "restart tau", ic.restart_tau);
	...
	//number of cycles after which sim restarts for GWs
	parseParameter(params, numparam, "restart cycle", sim.restart_cycle);
	...
	//add option to outout snapshot at every timestep
	
	if (parseParameter(params, numparam, "snapshot for every step", par_string))
	{
		if (par_string[0]=='Y' || par_string[0]=='y')
		{
			COUT <<"writing snapshots for every timestep" << endl;
			sim.snap_flag = 1;
		}
		else if (par_string[0]=='N' || par_string[0]=='n')
		{		
			COUT <<"NOT writing snapshots for every timestep" << endl;
			sim.snap_flag = 0;
		}
	}
	else
	{
		COUT <<"default: NOT writing snapshots for every timestep" << endl;
		sim.snap_flag = 0;
	}



	//add option to outout spectra at every timestep
	
	if (parseParameter(params, numparam, "pk for every step", par_string))
	{
		if (par_string[0]=='Y' || par_string[0]=='y')
		{
			COUT <<"writing spectra for every timestep" << endl;
			sim.pk_flag = 1;
		}
		else if (par_string[0]=='N' || par_string[0]=='n')
		{		
			COUT <<"NOT writing spectra for every timestep" << endl;
			sim.pk_flag = 0;
		}
	}
	else
	{
		COUT <<"default: NOT writing spectra for every timestep" << endl;
		sim.pk_flag = 0;
	}
	...
	parseParameter(params, numparam, "dcdm_fraction", cosmo.dcdm_fraction);
	if (parseParameter(params, numparam, "rate", cosmo.rate))
	{
		cosmo.rate = cosmo.rate*sim.boxsize/C_SPEED_OF_LIGHT;
	}
	...
	else
	{
		...
		COUT << " cosmological parameters are: Omega_m0 = " << cosmo.Omega_m << ", Omega_rad0 = " << cosmo.Omega_rad << ", Omega_Lambda0 = " << cosmo.Omega_Lambda << ", h = " << cosmo.h << endl;
